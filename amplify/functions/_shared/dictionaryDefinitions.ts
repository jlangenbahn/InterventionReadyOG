/**
 * Shared Bedrock dictionary writer used by the synchronous AppSync mutation
 * and the SQS mega-batch worker.
 */
import {
...
export const SYSTEM_PROMPT = `You are a lexicographer writing concise, accurate dictionary entries for an Orton-Gillingham reading program.
...
export const RETURN_RESULT_TOOL = {
  toolSpec: {
    name: 'return_result',
...
function toolUseInput(response: {
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    console.error('dictionary generation missing tool input', {
...
export function uniqueIds(raw: Array<string | null | undefined> | null | undefined) {
  const ids: string[] = [];
...
function parseEntries(raw: Record<string, unknown> | null, words: CatalogWord[]) {
  ...
  if (!list.length) {
    console.error('dictionary generation tool result had no entries array', {
  ...
  for (const row of list) {
    if (!row || typeof row !== 'object') {
      console.error('dictionary generation skipped non-object entry', { row });
      continue;
    }
    ...
    if (!catalog) {
      console.error('dictionary generation skipped entry with unknown wordId', {
        wordId,
        word: item.word,
      });
      continue;
    ...
    if (!parsed) {
      console.error('dictionary generation skipped malformed dictionary payload', {
        wordId: catalog.id,
        word: catalog.word,
...
export function needsDictionaryData(word: CatalogWord) {
  return !word.isNonsenseWord && !word.dictionaryData;
}

export async function runDictionaryGeneration(
  wordIdsInput: string[],
  ...
  const wordIds = uniqueIds(wordIdsInput);
  if (!wordIds.length) {
    return { createdCount: 0, message: 'No word ids were provided.', entries: [], processedCount: 0 };
  }
  if (wordIds.length > DICTIONARY_BATCH_LIMIT) {
    throw new Error(`Send at most ${DICTIONARY_BATCH_LIMIT} word ids per request.`);
  }

  const loaded = await loadWordsByIds(wordTable, wordIds);
  const words = loaded.filter(needsDictionaryData);
  if (!words.length) {
...
${JSON.stringify(words.map((word) => ({
  ...
  try {
    ...
    console.error('dictionary generation converse failed', err);
  ...
  if (!entries.length) {
    console.error('dictionary generation parsed zero writable entries', {
    ...
    return {
      createdCount: 0,
      message: `Processed 0 of ${wordIds.length} words`,
      entries: [],
      processedCount: wordIds.length,
    };
  }
  ...
  const written: DictionaryGenerationResult['entries'] = [];
  ...
  for (const row of entries) {
    ...
    try {
      ...
      console.error('dictionary generation failed to write dictionaryData', {
        wordId,
        word: catalog?.word,
